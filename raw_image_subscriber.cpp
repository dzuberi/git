...
public:
	...
	{
		//it(nh);
		mcdatacall_pub = nh.advertise<std_msgs::Empty>("/mcdatacall",1000);
		arduinosub = nh.subscribe("/arduino",1,&NodeClass::arduinoCallback,this);
		tivasub = nh.subscribe("/tiva",1,&NodeClass::tivaCallback,this);
		imagesub = nh.subscribe("/pg_12280229/image_raw",1,&NodeClass::imageCallback,this);
	}
	...
	void tivaCallback(const std_msgs::String::ConstPtr& msg) {
...
private:
	ros::NodeHandle nh;
	ros::Publisher mcdatacall_pub;
	ros::Subscriber arduinosub;
	ros::Subscriber tivasub;
